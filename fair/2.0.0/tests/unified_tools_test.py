...
def test_unstep_concentration():
    ...
    emissions_out, R_out = unifiedtools.unstep_concentration(a = a,\
                                                            dt = dt,\
...
def test_create_output_dataframes():
    ...
    gas_df, RF_df, T_df, alpha_df = unifiedtools.create_output_dataframes(inp_df, gas_np, RF_np, T_np, alpha_np, ext_forcing_np)
    ...
    assert not np.sum(RF_df.columns.tolist() != np.array(['a','b', 'External Forcing', 'Total']))
...
def test_return_np_function_arg_list():
    ...
    concentration_mode_arg_list = unifiedtools.return_np_function_arg_list(inp_df, cfg, concentration_mode = True)

    emission_mode_arg_list_compare = [  np.array([[2.14867530e+02, 4.40019020e+02, 1.70017052e+03, 3.50032202e+03, 2.35473520e+02], [3.00000000e+00, 1.63829600e+01, 6.36765930e+01, 1.80148890e+02, 2.43531850e+01]]),\
                                        np.array([ 1.00000000e+00,  2.17300000e-01]),\
                                        np.array([ 0.00000000e+00,  2.24000000e-01]),\
                                        np.array([ 0.00000000e+00,  2.82400000e-01]),\
                                        np.array([ 0.00000000e+00,  2.76300000e-01]),\
                                        np.array([ 9.15000000e+00,  1.00000000e+06]),\
                                        np.array([ 1.00000000e+00,  3.94400000e+02]),\
                                        np.array([ 1.00000000e+00,  3.65400000e+01]),\
                                        np.array([ 1.00000000e+00,  4.30400000e+00]),\
                                        np.array([ 9.07887400e+00,  2.86272960e+01]),\
                                        np.array([ 0.00000000e+00,  1.97730000e-02]),\
                                        np.array([ 0.00000000e+00,  4.33443300e+00]),\
                                        np.array([-2.87247000e-01,  0.00000000e+00]),\
                                        np.array([ 7.33822081e+02,  2.78000000e+02]),\
                                        np.array([ 3.51714000e-01,  4.68952344e-01]),\
                                        np.array([ 6.17360000e-02,  5.75438900e+00]),\
                                        ...
                                        np.array([1, 2, 4, 8, 8])]
    concentration_mode_arg_list_compare = [  np.array([[2.14867530e+02, 4.40019020e+02, 1.70017052e+03, 3.50032202e+03, 2.35473520e+02], [ 281.      ,  294.38296 ,  341.676593,  458.14889 , 302.353185]]),\
                                        np.array([ 1.00000000e+00,  2.17300000e-01]),\
                                        np.array([ 0.00000000e+00,  2.24000000e-01]),\
                                        np.array([ 0.00000000e+00,  2.82400000e-01]),\
                                        np.array([ 0.00000000e+00,  2.76300000e-01]),\
    ...
    for i in range(len(emission_mode_arg_list)):
        np.testing.assert_allclose(emission_mode_arg_list[i],emission_mode_arg_list_compare[i])
        np.testing.assert_allclose(concentration_mode_arg_list[i],concentration_mode_arg_list_compare[i])