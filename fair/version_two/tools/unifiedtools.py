...
def calculate_g(a,tau):
    g1 = ne.evaluate("sum( a * tau * ( 1. - ( 1. + 100/tau ) * exp(-100/tau) ), axis = 0)")
    g0 = np.exp( -1 * np.sum( a * tau * ( 1. - np.exp(-100/tau) ) , axis=0) / g1 )
    return g0, g1
...
def unstep_concentration(a,dt,alpha,tau,R_old, G_A):
    ...
    decay_rate = ne.evaluate("1/(alpha*tau)")
    decay_factor = ne.evaluate("exp(-dt*decay_rate)")
    emissions = (( G_A - np.sum(R_old*decay_factor,axis=0) ) / np.sum( a / decay_rate * ( 1. - decay_factor ) ,axis=0 ))
...
def create_output_dataframes(inp_df, gas_np, RF_np, T_np, alpha_np, ext_forcing_np):
    gas_df = convert_numpy_output_to_df(    gas_np,\
    ...
    RF_df = convert_numpy_output_to_df( RF_np,\
                                        np.append(inp_df.columns.values,np.array(['External Forcing', 'Total'])),\
                                        inp_df.columns.name,\
                                        inp_df.index.values,\
                                        inp_df.index.name)
    T_df = convert_numpy_output_to_df(  np.array([T_np]),\
                                        np.array(['T']),\
                                        None,\
                                        inp_df.index.values,\
                                        inp_df.index.name)
    alpha_df = convert_numpy_output_to_df(  alpha_np,\
                                            inp_df.columns.values,\
                                            inp_df.columns.name,\
                                            inp_df.index.values,\
                                            inp_df.index.name)
...
def return_np_function_arg_list(inp_df, cfg, concentration_mode = False):
    ...
    a1_np, a2_np, a3_np, a4_np, aer_conc_np, emis2conc_np, f1_np, f2_np, f3_np, PI_conc_np, r0_np, rA_np, rC_np, rT_np, tau1_np, tau2_np, tau3_np, tau4_np = convert_df_to_numpy(cfg['gas_params']).T
    ...
    inp_np = convert_df_to_numpy(inp_df) + ((PI_conc_np*aer_conc_np)[...,np.newaxis] if concentration_mode else 0)
