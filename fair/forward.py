...
from .ancil import cmip6_solar, cmip6_volcanic, historical_scaling, natural
from .constants import lifetime, molwt
...
def fair_scm(
    ...
    C_pi=np.array([278.0, 722.0, 273.0, 34.497] + [0.0] * 25 + [13.0975, 547.996]),
    E_pi=np.zeros(40),
    restart_in=False,
    restart_out=False,
    F_tropO3=0.0,
    ...
    efficacy=np.array([1.0] * 9 + [3.0] + [1.0] * 3),
    scale=None,
    oxCH4_frac=0.61,
    ghg_forcing="Etminan",
    scale_F2x=True,
    ...
    stevens_params=np.array([0.001875, 0.634, 60.0]),
    ...
    if iirf_h < iirf_max:
        warnings.warn(
            "iirf_h=%f, which is less than iirf_max (%f)" % (iirf_h, iirf_max),
            RuntimeWarning,
        )
    ...
    if useMultigas:
        ...
        if emissions_driven:
            ...
            if type(C) is not np.ndarray or C.shape[1] != ngas:
                raise ValueError("C timeseries should be a nt x %d numpy array" % ngas)
            if temperature_function == "Millar":
        ...
        if type(lifetimes) is np.ndarray:
            if len(lifetimes) != ngas:
                raise ValueError(
                    "custom GHG lifetime array must have " + str(ngas) + " elements"
                )
        else:
        ...
        if scale is None:
            ...
            raise ValueError(
                "in multi-gas mode, scale should be None, or a "
        ...
        if scale is None:
            ...
            raise ValueError(
                "in CO2-only mode, scale should be None, a " + "scalar or a (nt,) array"
            )
    ...
    if restart_in:
        ...
        if gir_carbon_cycle:
            raise NotImplementedError(
                "GIR carbon cycle not configured to " + "work with restarts"
            )
        else:
        ...
        if temperature_function == "Millar":
            ...
            raise (
                NotImplementedError(
                    "Restarts not implemented with Geoffroy " + "temperature function"
                )
            )
    ...
    if useMultigas:
        ...
        if type(emissions) is not bool:
            if tropO3_forcing[0].lower() == "s":  # stevenson
                F[0, iF_tro3] = ozone_tr.stevenson(
                    emissions[0, :],
                    C[0, 1],
                    T=np.sum(T_j[0, :]),
                ...
                F[0, iF_tro3] = ozone_tr.cmip6_stevenson(
                    emissions[0, :],
                    C[0, 1],
                    T=np.sum(T_j[0, :]),
                    feedback=useTropO3TFeedback,
        ...
        if type(emissions) is not bool:
            if contrail_forcing.lower()[0] == "n":  # from NOx emissions
                F[:, iF_cont] = contrails.from_aviNOx(
                    emissions, aviNOx_frac, F_ref=F_ref_aviNOx, E_ref=E_ref_aviNOx
                )
                ...
                raise ValueError(
                    "contrails must be one of 'NOx' (estimated "
        ...
        if type(emissions) is not bool:
            if aerosol_forcing.lower() == "stevens":
                ...
                aerosol_direct = aerosols.aerocom_direct(
                    emissions, beta=b_aero, E_pi=E_pi, diagnostics=diagnostics
                )
                if diagnostics == "AR6":
                ...
                if "ghan2" in aerosol_forcing.lower():
                    ...
                    ariaci[:, 1] = aerosols.ghan_indirect(
                        emissions,
                ...
                if diagnostics != "AR6":
                    ...
                    raise ValueError(
                        "AR6 diagnostics not compatible with "
                        + "externally forced aerosols"
                    )
            else:
                raise ValueError(
                    "aerosol_forcing should be one of 'stevens', "
        ...
        if type(emissions) is not bool:
            if bcsnow_forcing.lower()[0] == "e":
                F[:, iF_bcsn] = bc_snow.linear(
                    emissions - E_pi, F_ref=F_ref_BC, E_ref=E_ref_BC
                )
            else:
                F[:, iF_bcsn] = F_bcsnow
        else:
        ...
        if type(emissions) is not bool:
            if landuse_forcing.lower()[0] == "c":
                ...
                raise ValueError("landuse_forcing should be one of 'co2' or 'external'")
        else:
            F[:, iF_luch] = F_landuse
        ...
        if emissions_driven:
            if useMultigas:
                ...
                if tropO3_forcing[0].lower() == "s":
                    F[t, iF_tro3] = ozone_tr.stevenson(
                        emissions[t, :],
                        C[t, 1],
                        T=T[t - 1],
                        feedback=useTropO3TFeedback,
                    ...
                    F[t, iF_tro3] = ozone_tr.cmip6_stevenson(
                        emissions[t, :],
                        C[t, 1],
                        T=np.sum(T_j[t, :]),
                        feedback=useTropO3TFeedback,
                    ...
                    F[t, iF_tro3] = ozone_tr.regress(
                        emissions[t, :] - E_pi, beta=b_tro3
                    )
                ...
                if temperature_function == "Millar":
                    ...
                    T_j[t, :, :], heatflux[t], del_ohc, lambda_eff[t] = (
                        forcing_to_temperature(
                            T_j[t - 1, :, :],
                            np.sum(F[t - 1, :]),
                            np.sum(F[t, :]),
                            lambda_global=lambda_global,
        ...
        if temperature_function == "Geoffroy":
            if gir_carbon_cycle:
                return (
                    ...
                    ohc,
                    heatflux,
                    airborne_emissions / cumulative_emissions,
                )
            else:
